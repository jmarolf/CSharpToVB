...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax
    ...
    Public Class SyntaxNormalizer
        ...
        Private Property IndentationDepth As Integer
        ...
        Private Shared Function NeedsIndentAfterLineBreak(trivia As SyntaxTrivia) As Boolean
            Select Case trivia.Kind
                Case SyntaxKind.CommentTrivia,
                        ...
                        SyntaxKind.DocumentationCommentTrivia ' TODO May need to check for EOL
                    Return True
                Case Else
                    Return False
            End Select
        ...
        Private Shared Function NeedsSeparator(token As SyntaxToken, nextToken As SyntaxToken) As Boolean
            ...
            ' m( instead of m (
            If Not (token.Kind = SyntaxKind.CommaToken OrElse token.IsKind(SyntaxKind.AsKeyword)) AndAlso nextToken.Kind = SyntaxKind.OpenParenToken Then
                Return False
            End If
            ...
            If token.Kind = SyntaxKind.ColonToken AndAlso token.Parent.Kind = SyntaxKind.AttributeTarget Then
                Return True
            End If
            ' <Assembly: System.Copyright("(C) 2009")> instead of <Assembly :System.Copyright("(C) 2009")>
            If nextToken.Kind = SyntaxKind.ColonToken AndAlso nextToken.Parent.Kind = SyntaxKind.AttributeTarget Then
                Return False
            End If
            ...
            If token.Kind = SyntaxKind.XmlTextLiteralToken OrElse token.Kind = SyntaxKind.DocumentationCommentExteriorTrivia Then
                Return False
            End If
            ...
            If token.Kind = SyntaxKind.InterpolatedStringTextToken OrElse nextToken.Kind = SyntaxKind.InterpolatedStringTextToken Then
                Return False
            End If
            If token.Kind = SyntaxKind.CloseBraceToken AndAlso token.Parent.IsKind(SyntaxKind.Interpolation) Then
                Return False
            End If
            Return True
        End Function
        ...
        Private Sub AddLinebreaksAfterElementsIfNeeded(Of TNode As SyntaxNode)(
            ...
            For elementIndex As Integer = 0 To lastElementIndex
                ...
                If listElement.Kind = SyntaxKind.LabelStatement OrElse listElement.Kind = SyntaxKind.FieldDeclaration Then
                    ' always add line breaks after label or Field Declaration
        ...
        Private Function GetNextRelevantToken(token As SyntaxToken) As SyntaxToken
            ...
            Return If(_consideredSpan.Contains(nextToken.FullSpan), nextToken, CType(Nothing, SyntaxToken))
        End Function
            ...
            Try
                ...
                For i As Integer = 0 To triviaList.Count - 1
                    ...
                    If Trivia.Kind = SyntaxKind.WhitespaceTrivia Then
                        If _usePreserveCRLF Then
                            MinLeadingSpaces = Trivia.ToString.Count
                        End If
                        Continue For
                    ElseIf Trivia.Kind = SyntaxKind.LineContinuationTrivia OrElse
                            Trivia.FullWidth = 0 OrElse
                            (Trivia.Kind = SyntaxKind.EndOfLineTrivia And Not _usePreserveCRLF) Then
                        Continue For
                    End If
                    If Trivia.Kind = SyntaxKind.EndOfLineTrivia Then
                        If isTrailing Then
                    ...
                    Else
                        ...
                        If Trivia.Kind = SyntaxKind.DocumentationCommentExteriorTrivia OrElse NeedExtraSpace Then
                ...
                If currentTriviaList.Count = 0 Then
                    Return If(_useElasticTrivia, SyntaxFactory.TriviaList(SyntaxFactory.ElasticMarker), CType(Nothing, SyntaxTriviaList))
        ...
        Public Overrides Function VisitLabelStatement(node As LabelStatementSyntax) As SyntaxNode
            ...
            Dim previousIndentationDepth As Integer = IndentationDepth
            IndentationDepth = 0
        ...
        Public Overrides Function VisitMultiLineIfBlock(node As MultiLineIfBlockSyntax) As SyntaxNode
            ...
            previousNode = If(node.Statements.Any(), DirectCast(node.Statements.Last(), VisualBasicSyntaxNode), DirectCast(node.IfStatement, VisualBasicSyntaxNode))
        ...
        Public Overrides Function VisitToken(token As SyntaxToken) As SyntaxToken
            'For Debugging
            'If token.ToString = "Extension" Then
            '    Stop
            'End If
            ...
            Try
                ...
                newToken = If(_useDefaultCasing AndAlso token.IsKeyword(), SyntaxFactory.Token(token.Kind), token)
                ...
                ' for the next token
                Dim numLineBreaksAfter As Integer
                numLineBreaksAfter = If(_usePreserveCRLF, LineBreaksBetween(token, nextToken), If(LineBreaksBetween(token, nextToken) > 0, 1, 0))
