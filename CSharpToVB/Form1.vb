...
Public Class Form1
    ...
    Private Sub ButtonStop_MouseLeave(sender As Object, e As EventArgs) Handles ButtonStop.MouseLeave
        LocalUseWaitCutsor(MeForm:=Me, Enable:=Not StopRequested)
    End Sub
    ...
    Private Function Convert_Compile_Colorize(RequestToConvert As ConvertRequest, OptionalReferences() As MetadataReference) As Boolean
        ResultOfConversion = ConvertInputRequest(RequestToConvert, OptionalReferences)
    ...
    Private Sub ConvertSnipitToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ConvertSnipitToolStripMenuItem.Click
        ...
        Convert_Compile_Colorize(RequestToConvert:=RequestToConvert, OptionalReferences:=References)
        ConvertFolderToolStripMenuItem.Enabled = True
        SetButtonStopAndCursor(MeForm:=Me, StopButton:=ButtonStop, VisibleValue:=False)
    End Sub
...
#Enable Warning CC0014 ' Use Ternary operator.
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If File.Exists(My.Settings.LastFileNameWithPath) Then
            RecentFile1ToolStripMenuItem.Text = My.Settings.LastFileNameWithPath
        Else
            My.Settings.LastFileNameWithPath = ""
            My.Settings.Save()
        End If
    ...
    Private Sub OpenFileToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles OpenFileToolStripMenuItem1.Click
        ...
        If FileOpenResult = DialogResult.OK Then
            ...
            Dim FileStream As Stream = File.OpenRead(path:=RecentFileNameWithPath)
            Dim InputLines As Integer = LoadInputBufferFromStream(LanguageExtension:=LanguageExtension, fileStream:=FileStream, SkipAutoGenerated:=False)
    ...
    Private Sub OpenProjectToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles OpenProjectToolStripMenuItem.Click
        ...
        OpenFileDialog1.DefaultExt = LanguageExtension
        OpenFileDialog1.FileName = ""
        OpenFileDialog1.Filter = If(LanguageExtension = "vb", "VB Project File (*.vbproj)|*.vbproj", "C# Project File (*.csproj)|*.csproj")
        SaveFileDialog1.FilterIndex = 0
        OpenFileDialog1.Multiselect = False
        OpenFileDialog1.ReadOnlyChecked = True
        OpenFileDialog1.Title = $"Open {LanguageExtension.ToUpper} Project file"
        OpenFileDialog1.ValidateNames = True
            ...
            Using Workspace As MSBuildWorkspace = MSBuildWorkspace.Create()
                ...
                If currentProject.HasDocuments Then
                    For Each document As Document In currentProject.Documents
                        ...
                        ProcessFile(document.FilePath, LanguageExtension, currentProject.MetadataReferences.ToArray)
                    Next document
                End If
            End Using
            SetButtonStopAndCursor(MeForm:=Me, StopButton:=ButtonStop, VisibleValue:=False)
    ...
    Private Function ProcessDirectory(targetDirectory As String, LastFileNameWithPath As String, LanguageExtension As String, ByRef FilesProcessed As Integer) As Boolean
        Try
            ...
            Return ProcessDirectoriesModule.ProcessDirectory(targetDirectory:=targetDirectory, MeForm:=Me, StopButton:=ButtonStop, RichTextBoxFileList:=RichTextBoxFileList, LastFileNameWithPath:=LastFileNameWithPath, LanguageExtension:=LanguageExtension, FilesProcessed:=FilesProcessed, ProcessFile:=AddressOf ProcessFile)
        Catch ex As Exception
            ' don't crash on exit
            Stop
        Finally
    ...
    Private Function ProcessFile(ByVal PathWithFileName As String, LanguageExtension As String, OptionalReferences() As MetadataReference) As Boolean
        ...
        If lines > 0 Then
            RequestToConvert.SourceCode = RichTextBoxConversionInput.Text
            If Not Convert_Compile_Colorize(RequestToConvert, OptionalReferences) Then
                RestoreMonitorSettings()
    ...
    Private Sub RecentFile1ToolStripMenuItem_MouseDown(sender As Object, e As MouseEventArgs) Handles RecentFile1ToolStripMenuItem.MouseDown
        If e.Button = Windows.Forms.MouseButtons.Right Then
            Clipboard.SetText(text:=RecentFile1ToolStripMenuItem.Text)
        End If
    End Sub
